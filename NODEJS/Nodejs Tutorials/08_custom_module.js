...
function userRequestHandler(req, res) {
  if (req.url.toLowerCase() === "/login") {
    ...
    req.on("end", () => {
      ...
      fs.writeFileSync("user_for_program8.txt", JSON.stringify(finalObj)); // will will inject the data to new file
    });
...
module.exports = userRequestHandler; // default export and below are named exports
...
[Note while importing use {moduleName} which destructures modulename ]
*/
