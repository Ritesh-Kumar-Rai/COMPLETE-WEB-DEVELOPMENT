        ...
        <section className="max-w-7xl mx-auto px-10">
            <div className="flex items-center justify-between gap-4 flex-wrap">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    {categories?.map(cat => <Badge key={cat} variant={`${cat === 'All' ? 'default' : 'outline'}`} >{cat}</Badge>)}
                </div>
            </div>
            <div className="my-10">
                {/* All product Cards will be render below: */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-8">
                    {homeData.map(product => <ProductCard key={product.id} imgSrc={product.imgSrc} title={product.title} category={product.category} rating={product.rating} price={product.price} discountPrice={product.discountPrice} />)}
                </div>
